...
// exports.postById = (req, res, next, id) => {
//   Post.findById(id).populate("postedBy", "_id name").populate('comments', 'text created').populate('comments.postedBy', '_id name').exec((err, post) => {
//     if(err || !post) {
//       return res.status(400).json({
//         error: err
//       })
...
//   const posts = Post.find().populate("postedBy", "_id name").populate('comments', 'text created').populate('comments.postedBy', '_id name')
//     .select("_id title body created likes").sort({created: -1})
//     .then(posts => {
//       res.json(posts);
